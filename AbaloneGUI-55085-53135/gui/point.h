 ...
 */
template<class T>
bool coincide(const point<T> & p1, const point<T> & p2,
              T precision = 10E-9L)
{
...
{
    return std::abs(p1.first - p2.first) + std::abs(p1.second -
            p2.second);
}
...
{
    return make_point(std::hypot(p.first, p.second), std::atan2(p.second,
                      p.first));
}
...
{
    return make_point(p.first * std::cos(p.second),
                      p.first * std::sin(p.second));
}
 ...
 */
template<class T>
point<T> polar_translate(const point<T> & p, const T & mod,
                         const T & arg)
{

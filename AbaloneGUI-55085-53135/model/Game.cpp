...
{
    board_ = Board();
    current_ = Player(PlayerColor::WHITE);
    opponent_ = Player(PlayerColor::BLACK);
...
void Game::play(std::string abaPro)
{
    if (abaPro.length() == 4)
    {
        lineMovement(abaPro);
    }
    else if (abaPro.length() == 6)
    {
        diagonalMovement(abaPro);
    }
    else
    {
...
void Game::lineMovement(std::string abaPro)
{
    Position selected{stringToPosition(abaPro.substr(0, 2))};
    posCurrentColor(selected);
    Direction direction{verifyDirection(selected, abaPro.substr(2, 2))};
...
void Game::diagonalMovement(std::string abaPro)
{
    Position posFirstMarble{stringToPosition(abaPro.substr(0, 2))};
    Position posLastMarble{stringToPosition(abaPro.substr(2, 2))};
    Direction direction{verifyDirection(posFirstMarble, abaPro.substr(4, 2))};
